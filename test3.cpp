...
void AddTransaction() {
    ...
    transactions[++tRear] = {id, customer, qty, std::chrono::system_clock::now(), false};
    tCount++;
...
void DisplayTransactions() {
    ...
    if (tCount == 0) {
        ...
        cout << "|| ID    | Customer        | Qty |  Status  ||" << endl;
        cout << "||-------|-----------------|-----|----------||" << endl;
        for (int i = tFront; i <= tRear; ++i) {
            ...
            cout << status;
            int statPad = 9 - status.length();
            for (int j = 0; j < statPad; ++j) cout << " ";
            cout << "||" << endl;
